...
#define CLIENT_DOWNLOAD_DIR "/media/gy/win/VSCODE/class/项目/网盘/下载目录" // 客户端下载目录
...
bool dealDownload(int socket, Package *package)
{
    ...
    // 使用客户端下载目录
    string savePath = CLIENT_DOWNLOAD_DIR;
    ...
    FILE *fp = fopen(fullSavePath.c_str(), "w");
    if (!fp)
    {
        perror("文件创建失败");
        return false;
